...
{
    [Range(0, 20f)]
    public float locomotionBlendSpeed = 20f;
    [Range(0, 20f)]
    public float targetLockBlendSpeed = 20f;
    [Range(0f, 80f)]
    ...
    private void Move()
    {
        ...
        if (_input.move != Vector2.zero)
        {
            if (strafe && !_input.sprint)
            {
                ...
                transform.forward += Vector3.Lerp(transform.forward, rotationOffset, Time.deltaTime * targetLockRotationSpeed);
            }
            else 
            {
                ...
                if (directBlendDelay > 1) directBlendDelay = 1;
            }
        }
        else
        {
        ...
        targetDirection = Vector3.Lerp(targetDirection, directMovement, DirectMoveBlend * directBlendDelay);
        ...
        if (strafe && !_input.sprint)
        {
            ...
            velocity = Vector3.Lerp(velocity, (transform.forward * _input.move.y) +
               (transform.right * _input.move.x), locomotionBlendSpeed * Time.deltaTime);

            _controller.Move(velocity.normalized * (_speed * Time.deltaTime) +
